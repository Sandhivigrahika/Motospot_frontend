...
export default function ProfileScreen() {
  ...
  // ── Queries ────────────────────────────────────────────────────────────────
  const {
    data: bikes = [], isLoading: bikesLoading,
    refetch: refetchBikes, isFetching: bikesFetching,
  } = useQuery({ queryKey: ['myBikes'], queryFn: fetchMyBikes });

  const {
    data: addresses = [], isLoading: addressesLoading,
    refetch: refetchAddresses, isFetching: addressesFetching,
  ...
  // ── Phone Update ───────────────────────────────────────────────────────────
  const openPhoneEdit = () => { setPhoneInput(user?.phone ?? ''); setPhoneModal(true); };
  ...
  const savePhone = async () => {
    ...
    try {
      ...
      if (!res.ok) { Alert.alert('Error', text || `Status ${res.status}`); return; }
  ...
  // ── Email Update ───────────────────────────────────────────────────────────
  const openEmailEdit = () => { setEmailInput(user?.email ?? ''); setEmailModal(true); };
  ...
  const saveEmail = async () => {
    ...
    try {
      ...
      let res = await fetch(`${BASE}/user/email`, {
        method: 'PUT', headers, body: JSON.stringify({ email }),
      });
            ...
            bikes.map((bike: any) => (
              ...
              >
                <View style={styles.itemLeft}>
                  <Text style={styles.itemPrimary}>{bike.company_name} {bike.model_name}</Text>
            ...
            addresses.map((addr: any) => (
              ...
              >
                <View style={styles.itemLeft}>
                  ...
                  <Text style={styles.itemSub} numberOfLines={1}>{addr.address_line}, {addr.city}</Text>
                </View>
                <Text style={styles.rowChevron}>›</Text>
              </TouchableOpacity>
            ))
...
// ── Reusable Edit Modal ──────────────────────────────────────────────────────
function EditModal({ visible, title, value, onChange, onSave, onClose, loading, placeholder, keyboardType }: any) {
  return (
...
const styles = StyleSheet.create({
  header: {
    ...
    paddingHorizontal: 20, paddingVertical: 16, backgroundColor: '#f3f4f6',
  },
  headerTitle: { fontSize: 26, fontWeight: '800', color: '#111827' },
  logoutBtn: { backgroundColor: '#2563eb', paddingHorizontal: 20, paddingVertical: 9, borderRadius: 10 },
  logoutText: { color: '#fff', fontWeight: '700', fontSize: 15 },
  divider: { height: 1, backgroundColor: '#e5e7eb' },
  scroll: { flex: 1 },
  fieldRow: { paddingHorizontal: 20, paddingVertical: 18, backgroundColor: '#f3f4f6' },
  ...
  // Support
  supportRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#e5e7eb' },
  supportIcon: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#eff6ff', alignItems: 'center', justifyContent: 'center', marginRight: 14 },
  supportEmoji: { fontSize: 20 },
  supportText: { flex: 1 },
  supportLabel: { fontSize: 13, color: '#6b7280', fontWeight: '600', marginBottom: 3 },
  supportValue: { fontSize: 15, fontWeight: '600', color: '#111827' },
  ...
  modalInput: { borderWidth: 1, borderColor: '#d1d5db', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, fontSize: 16, color: '#111827', marginBottom: 20 },
  modalBtns: { flexDirection: 'row', gap: 12 },
  cancelBtn: { flex: 1, paddingVertical: 13, borderRadius: 10, borderWidth: 1, borderColor: '#d1d5db', alignItems: 'center' },
  cancelText: { color: '#6b7280', fontWeight: '600', fontSize: 15 },
  saveBtn: { flex: 1, backgroundColor: '#16a34a', paddingVertical: 13, borderRadius: 10, alignItems: 'center' },
